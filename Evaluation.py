...
class precision_recall_calculator:
    ...
    def create_user_test_sample(self, percentage):
        ...
        users_test_and_training = list(
            set(self.test_data["user_id"].unique()).intersection(
        ...
        self.users_test_sample = self.remove_percentage(
            users_test_and_training, percentage
        )
    ...
    def calculate_precision_recall(self):
        ...
        for N in cutoff_list:
            ...
            for user_id in self.users_test_sample:
                ism_hitset = self.test_dict[user_id].intersection(
                    set(self.ism_training_dict[user_id][0:N])
                )
                pm_hitset = self.test_dict[user_id].intersection(
                    set(self.pm_training_dict[user_id][0:N])
                )
